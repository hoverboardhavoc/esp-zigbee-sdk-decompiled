/*
 * Last changed at upstream commit 6fb04b5b77a96f6ce5cb3daff360f54f13d8db16
 * https://github.com/espressif/esp-zigbee-sdk/commit/6fb04b5b77a96f6ce5cb3daff360f54f13d8db16
 * Upstream date: 2023-06-09 17:12:37 +0800
 * Upstream subject: esp-zigbee-sdk: release esp-zigbee-sdk v0.6.2(2626797)
 * Source: libesp_zb_api_zczr -> esp_zigbee_attribute.o -> esp_zb_illuminance_meas_cluster_add_attr
 *
 * (C) Espressif, Apache License 2.0.
 * Derivative work (this file): mechanical decompile via Ghidra (NSA, Apache 2.0).
 * Decompiler output may be incomplete or differ from original semantics.
 */

int esp_zb_illuminance_meas_cluster_add_attr(int param_1,uint param_2,undefined1 *param_3)

{
  short *__ptr;
  int iVar1;
  undefined1 *puVar2;
  undefined4 uVar3;
  int aiStack_24 [2];
  
  __ptr = (short *)malloc(0x10);
  aiStack_24[0] = 0;
  iVar1 = esp_zb_attr_list_get_tail(param_1,0x400,param_2,aiStack_24);
  if (aiStack_24[0] != 0) {
    free(__ptr);
    return aiStack_24[0];
  }
  *__ptr = (short)param_2;
  if (param_2 < 4) {
    if ((short)param_2 == 0) {
      if (param_2 != 0) goto _L0;
      *(undefined1 *)((int)__ptr + 3) = 5;
      *(undefined1 *)(__ptr + 1) = 0x21;
      puVar2 = (undefined1 *)malloc(2);
      __ptr[3] = (short)puVar2;
      __ptr[4] = (short)((uint)puVar2 >> 0x10);
      *puVar2 = *param_3;
      puVar2[1] = param_3[1];
    }
    else {
      *(undefined1 *)((int)__ptr + 3) = 1;
      *(undefined1 *)(__ptr + 1) = 0x21;
      puVar2 = (undefined1 *)malloc(2);
      __ptr[3] = (short)puVar2;
      __ptr[4] = (short)((uint)puVar2 >> 0x10);
      *puVar2 = *param_3;
      puVar2[1] = param_3[1];
    }
  }
  else {
    if (param_2 != 4) {
_L0:
      uVar3 = esp_log_timestamp();
      esp_log_write(1,"ESP_ZIGBEE_ATTRIBUTE",&_L0,uVar3,"ESP_ZIGBEE_ATTRIBUTE");
      free(__ptr);
      return 0x102;
    }
    *(undefined1 *)((int)__ptr + 3) = 1;
    *(undefined1 *)(__ptr + 1) = 0x30;
    puVar2 = (undefined1 *)malloc(2);
    __ptr[3] = (short)puVar2;
    __ptr[4] = (short)((uint)puVar2 >> 0x10);
    *puVar2 = *param_3;
    puVar2[1] = param_3[1];
  }
  __ptr[2] = -1;
  __ptr[5] = 0x400;
  __ptr[6] = 0;
  __ptr[7] = 0;
  if (iVar1 == 0) {
    *(short **)(param_1 + 0xc) = __ptr;
  }
  else {
    *(short **)(iVar1 + 0xc) = __ptr;
  }
  return 0;
}

