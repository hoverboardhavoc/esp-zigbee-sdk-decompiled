 ...
 * Source: libesp_zb_api_zczr -> esp_zigbee_cluster.o -> esp_zb_cluster_list_add_time_cluster
 *
...
undefined4 esp_zb_cluster_list_add_time_cluster(int param_1,int param_2,uint param_3)

{
  ...
  if (param_1 == 0) {
  ...
  else if (*(short *)(*(int *)(param_2 + 0xc) + 10) == 10) {
    psVar1 = *(short **)(param_1 + 0x10);
    do {
      psVar2 = psVar1;
      if (psVar2 == (short *)0x0) break;
      if (*psVar2 == 10 && *(byte *)(psVar2 + 4) == param_3) {
        uVar5 = esp_log_timestamp();
        esp_log_write(1,"ESP_ZIGBEE_CLUSTER",&_LC5,uVar5,"ESP_ZIGBEE_CLUSTER",10);
        return 0x102;
      }
    ...
    local_30 = CONCAT22(uVar3,10);
    if (param_3 == 1) {
      puVar6 = &zb_zcl_time_init_server;
    }
    else if (param_3 == 2) {
      puVar6 = &zb_zcl_time_init_client;
    }
    else {
