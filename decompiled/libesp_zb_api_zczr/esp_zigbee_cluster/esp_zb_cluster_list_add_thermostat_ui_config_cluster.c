...

{
  ...
  iVar1 = esp_zb_cluster_list_get_tail(param_1,param_2,0x204,param_3,aiStack_2c);
  if (aiStack_2c[0] == 0) {
    uVar2 = zcl_convert_attr_list_to_array(param_2);
    if (param_3 == 1) {
      puVar3 = &zb_zcl_thermostat_ui_config_init_server;
    }
    else if (param_3 == 2) {
      puVar3 = &zb_zcl_thermostat_ui_config_init_client;
    }
    else {
