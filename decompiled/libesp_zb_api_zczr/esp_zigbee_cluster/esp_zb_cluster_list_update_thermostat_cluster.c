 ...
 * Source: libesp_zb_api_zczr -> esp_zigbee_cluster.o -> esp_zb_cluster_list_update_thermostat_cluster
 *
...
int esp_zb_cluster_list_update_thermostat_cluster(int param_1,int param_2,int param_3)

{
  ...
  iVar3 = esp_zb_cluster_list_check(param_1,param_2,0x201);
  if (iVar3 == 0) {
    ...
    while (__ptr_00 != (short *)0x0) {
      if (*__ptr_00 == sVar1) {
        local_30 = CONCAT22(uVar2,0x201);
        if (param_3 == 1) {
          puVar5 = &zb_zcl_thermostat_init_server;
        }
        else if (param_3 == 2) {
          puVar5 = &zb_zcl_thermostat_init_client;
        }
        else {
