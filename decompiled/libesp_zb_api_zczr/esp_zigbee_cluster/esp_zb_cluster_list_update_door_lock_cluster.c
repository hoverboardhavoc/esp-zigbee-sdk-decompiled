...
undefined4 esp_zb_cluster_list_update_door_lock_cluster(int param_1,int param_2,int param_3)

{
  ...
  if (param_1 == 0) {
    ...
    while (__ptr != (short *)0x0) {
      if (*__ptr == 0x101) {
        local_30 = CONCAT22(uVar2,0x101);
        if (param_3 == 1) {
          puVar5 = &zb_zcl_door_lock_init_server;
        }
        else if (param_3 == 2) {
          puVar5 = &zb_zcl_door_lock_init_client;
        }
        else {
    ...
    uVar4 = esp_log_timestamp();
    esp_log_write(1,"ESP_ZIGBEE_CLUSTER",&_LC25,uVar4,"ESP_ZIGBEE_CLUSTER");
    uVar4 = 0x102;
  }
  return uVar4;
}
