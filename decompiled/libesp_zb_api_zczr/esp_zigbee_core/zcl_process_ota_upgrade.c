  ...
  if (bVar1 == 3) {
    ...
    do {
      ...
      if (iVar7 != 0) {
        uVar4 = esp_log_timestamp();
        uVar3 = esp_err_to_name(iVar7);
        esp_log_write(1,"ESP_ZIGBEE_CORE",&_LC12,uVar4,"ESP_ZIGBEE_CORE",uVar3);
      }
      uVar4 = esp_log_timestamp();
      esp_log_write(3,"ESP_ZIGBEE_CORE",&_LC13,uVar4,"ESP_ZIGBEE_CORE");
      esp_restart();
_L0:
      uVar4 = esp_log_timestamp();
      esp_log_write(1,"ESP_ZIGBEE_CORE",&_LC10,uVar4,"ESP_ZIGBEE_CORE",iVar7);
    } while( true );
  }
  if (bVar1 < 4) {
    if (bVar1 == 1) {
      ...
      if ((uVar6 < 0x38) && (uVar5 = *(byte *)(param_1 + 0x14) + uVar6, 0x38 < uVar5)) {
        ...
        if (update_partition == 0) {
          __assert_func("//home/xieqinan/ESP/esp-zboss/components/esp_zb_sdk/src/esp_zigbee_core.c",
                        0x289,"zcl_process_ota_upgrade","update_partition != NULL");
        }
        else {
