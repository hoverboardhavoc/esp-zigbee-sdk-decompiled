/*
 * Last changed at upstream commit 02e71c61b42f2e0f80074362fea601f2245ed9d0
 * https://github.com/espressif/esp-zigbee-sdk/commit/02e71c61b42f2e0f80074362fea601f2245ed9d0
 * Upstream date: 2026-04-16 12:25:02 +0800
 * Upstream subject: change: update esp-zigbee-lib 2.x (bce53822)
 * Source: libesp-zigbee-core.zczr.release -> thermostat_desc.o -> ezb_zcl_thermostat_cluster_desc_add_attr
 *
 * (C) Espressif, Apache License 2.0.
 * Derivative work (this file): mechanical decompile via Ghidra (NSA, Apache 2.0).
 * Decompiler output may be incomplete or differ from original semantics.
 */

/* WARNING: Unknown calling convention */

ezb_err_t ezb_zcl_thermostat_cluster_desc_add_attr
                    (ezb_zcl_cluster_desc_t cluster_desc,uint16_t attr_id,void *value)

{
  ezb_err_t eVar1;
  undefined2 in_register_0000202e;
  uint uVar2;
  uint uVar3;
  undefined4 uVar4;
  uint uVar5;
  
  uVar2 = CONCAT22(in_register_0000202e,attr_id);
  if (cluster_desc == (ezb_zcl_cluster_desc_t)0x0) {
    return 2;
  }
                    /* WARNING: Load size is inaccurate */
  if (*cluster_desc != 0x201) {
    return 2;
  }
  uVar5 = (uint)*(byte *)((int)cluster_desc + 2);
  uVar3 = 1;
  if (1 < (uVar5 - 1 & 0xffff)) {
    uVar5 = 0;
    uVar3 = 0;
    uVar2 = __assert_func(0,0,value);
  }
  if (uVar5 != uVar3) {
    return 6;
  }
  uVar4 = 0x20;
  if (uVar2 != 0x20) {
    if (uVar2 < 0x21) {
      if (uVar2 < 0x13) {
        if (0x10 < uVar2) {
_L0:
          uVar4 = 0x29;
          goto _L0;
        }
        if (uVar2 < 9) {
          if (6 < uVar2) goto _L0;
          if ((uVar2 == 0) || (uVar2 != 2)) goto _L0;
        }
        else if (uVar2 != 9) {
          uVar4 = 0x28;
          if (uVar2 != 0x10) {
            return 6;
          }
          goto _L0;
        }
      }
      else if (uVar2 != 0x1a) {
        if (uVar2 < 0x1b) {
          if (uVar2 == 0x19) {
            uVar4 = 0x28;
            goto _L0;
          }
          goto _L0;
        }
        if ((uVar2 != 0x1c) && (0x1c < uVar2)) {
          if (uVar2 == 0x1d) goto _L0;
          if (uVar2 != 0x1e) {
            return 6;
          }
        }
        goto _L0;
      }
_L0:
      uVar4 = 0x18;
      goto _L0;
    }
    if (uVar2 == 0x34) goto _L0;
    if (uVar2 < 0x35) {
      if (uVar2 == 0x29) {
        uVar4 = 0x19;
        goto _L0;
      }
      if (uVar2 < 0x2a) {
        if (uVar2 == 0x25) goto _L0;
        if (0x25 < uVar2) {
          return 6;
        }
        if (uVar2 == 0x24) {
_L0:
          uVar4 = 0x21;
          goto _L0;
        }
        if (uVar2 != 0x23) goto _L0;
      }
      else {
        if (uVar2 == 0x32) {
          uVar4 = 0xe2;
          goto _L0;
        }
        if (uVar2 == 0x33) {
          return 6;
        }
        if (uVar2 != 0x30) {
          if (uVar2 != 0x31) {
            return 6;
          }
          goto _L0;
        }
      }
    }
    else {
      if (uVar2 == 0x44) {
        uVar4 = 0x1b;
        goto _L0;
      }
      if (uVar2 < 0x45) {
        if ((uVar2 == 0x3a) || (uVar2 < 0x3b)) {
_L0:
          uVar4 = 0x20;
          goto _L0;
        }
        if (uVar2 == 0x41) goto _L0;
        if ((uVar2 < 0x42) && (uVar2 != 0x40)) {
          return 6;
        }
      }
      else {
        if (uVar2 == 0xeff0) {
          uVar4 = 0;
          goto _L0;
        }
        if (0xeff0 < uVar2) {
          return 6;
        }
        if (uVar2 == 0x46) goto _L0;
        if ((uVar2 & 0xfffffffd) != 0x45) {
          return 6;
        }
      }
    }
  }
_L0:
  uVar4 = 0x30;
_L0:
  eVar1 = ezb_zcl_cluster_desc_add_manuf_attr(uVar4,0);
  return eVar1;
}

