...

{
  ...
  zmsg_get_footer(msg,&aps_hdr.ext_hdr.block_nr,0x1c);
...
_L0:
    ...
    if ((req._20_4_ & 2) == 0) {
      error = aps_send_frame(uStack_5e,(aps_apsde_data_req_t *)&aps_hdr.ext_hdr.block_nr);
    }
    else {
      error = aps_retrans_send_msg(uVar7 & 0xff,&aps_hdr.ext_hdr.block_nr);
    }
    if (error == 0) goto _L0;
  }
  aps_send_confirm(msg,error);
_L0:
  tasklet_post(ctx);
  return;
}
