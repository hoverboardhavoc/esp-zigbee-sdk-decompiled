...

{
  ...
  if (((cmd_req == (zcl_read_attr_cmd_t *)0x0) || ((cmd_req->payload).attr_number == '\0')) ||
     ((cmd_req->payload).attr_field == (uint16_t *)0x0)) {
    iVar1 = 2;
  }
  else {
    iVar1 = -1;
    iVar2 = zcl_packet_init(auStack_38,*(uint *)&(cmd_req->cmd_ctrl).fc & 1);
    if (iVar2 == 0) {
      ...
      if (iVar1 == 0) {
        while ((iVar2 < (int)(uint)(cmd_req->payload).attr_number &&
          ...
          iVar1 = zmsg_append_le16((zmsg_t *)packet._32_4_,puVar4[iVar2]);
          if (iVar1 != 0) goto _L0;
          iVar2 = iVar2 + 1;
        }
