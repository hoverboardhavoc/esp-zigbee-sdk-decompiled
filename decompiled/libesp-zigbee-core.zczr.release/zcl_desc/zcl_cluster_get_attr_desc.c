...

{
  ...
  if (cluster_desc != (zcl_cluster_desc_t *)0x0) {
    while (cluster_desc = (zcl_cluster_desc_t *)cluster_desc->attr_list,
          cluster_desc != (zcl_cluster_desc_t *)0x0) {
      if ((uint)cluster_desc->cluster_id == CONCAT22(in_register_0000202e,attr_id)) {
        if (CONCAT22(in_register_00002032,manuf_code) == 0) {
          return (zcl_attr_desc_t *)cluster_desc;
        }
        if (((cluster_desc->field_0x3 & 0x20) != 0) &&
