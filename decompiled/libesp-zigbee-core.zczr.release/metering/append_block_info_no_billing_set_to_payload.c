...

{
  ...
  if ((iVar3 == 0) &&
     (eVar4 = zmsg_append_u8(payload,block_info_no_billing_set->num_of_tiers_in_use), eVar4 == 0)) {
    ...
    if (iVar3 != 0) goto _L0;
    eVar4 = zmsg_append_u8(payload,block_info_no_billing_set->
                                   num_of_tiers_and_block_thresholds_in_use);
    if (eVar4 == 0) {
      puVar5 = block_info_no_billing_set->tier_block_summation;
      if ((block_info_no_billing_set->num_of_tiers_and_block_thresholds_in_use == 0) ||
         (eVar2 = '\x01', puVar5 != (uint64_t *)0x0)) {
        puVar1 = puVar5 + block_info_no_billing_set->num_of_tiers_and_block_thresholds_in_use;
        do {
          if (puVar5 == puVar1) {
            return '\0';
          }
