 ...
 * Source: libesp-zigbee-core.zczr.release -> color_control.o -> ezb_zcl_color_control_move_to_hue_cmd_req
 *
...
ezb_err_t ezb_zcl_color_control_move_to_hue_cmd_req
                    (ezb_zcl_color_control_move_to_hue_cmd_t *cmd_req)

{
  ...
  if ((cmd_req == (ezb_zcl_color_control_move_to_hue_cmd_t *)0x0) ||
     (iVar1 = zcl_packet_init(auStack_38,0), iVar1 != 0)) {
    iVar1 = -1;
  }
  else {
    memcpy(auStack_50,cmd_req,10);
    iVar1 = zcl_cmd_to_packet(auStack_38,0,0,(cmd_req->cmd_ctrl).dis_default_rsp,0,0,0x300);
    if ((((iVar1 == 0) &&
