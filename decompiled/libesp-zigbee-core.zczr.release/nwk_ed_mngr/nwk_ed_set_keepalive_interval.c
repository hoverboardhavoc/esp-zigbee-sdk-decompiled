/*
 * Last changed at upstream commit 0dbfa9988ffc315d4d533fc462a8328b10d4d371
 * https://github.com/espressif/esp-zigbee-sdk/commit/0dbfa9988ffc315d4d533fc462a8328b10d4d371
 * Upstream date: 2026-07-09 09:00:50 +0000
 * Upstream subject: change: update esp-zigbee-lib (170bcb5a)
 * Source: libesp-zigbee-core.zczr.release -> nwk_ed_mngr.o -> nwk_ed_set_keepalive_interval
 *
 * (C) Espressif, Apache License 2.0.
 * Derivative work (this file): mechanical decompile via Ghidra (NSA, Apache 2.0).
 * Decompiler output may be incomplete or differ from original semantics.
 */

/* WARNING: Removing unreachable block (ram,0x0001092c) */

void nwk_ed_set_keepalive_interval(int param_1)

{
  uint uVar1;
  undefined2 uVar2;
  undefined4 uVar3;
  uint uVar4;
  int iVar5;
  uint uVar6;
  undefined4 uStack_24;
  
  iVar5 = core_globals_get();
  if (*(int *)(iVar5 + 0xaf4) == param_1) {
    return;
  }
  iVar5 = core_globals_get();
  *(int *)(iVar5 + 0xaf4) = param_1;
  iVar5 = core_globals_get();
  if ((*(byte *)(iVar5 + 0xb16) & 0x20) != 0) {
    nwk_pim_schedule_next_poll(1);
  }
  iVar5 = core_globals_get();
  if (-1 < *(char *)(iVar5 + 0xaf8)) {
    return;
  }
  iVar5 = core_globals_get();
  milli_timer_stop(iVar5 + 0xae4);
  iVar5 = core_globals_get();
  uVar1 = *(uint *)(iVar5 + 0xaf8) >> 7 & 1;
  if (uVar1 == 0) {
    return;
  }
  iVar5 = core_globals_get();
  if ((*(byte *)(iVar5 + 0xa28) & 1) == 0) {
    iVar5 = core_globals_get();
    if ((*(byte *)(iVar5 + 0xa28) & 2) == 0) {
      iVar5 = core_globals_get();
      if (*(char *)(iVar5 + 0x9ee) != '\0') {
        nwk_pim_stop_poll();
      }
      goto _L0;
    }
    iVar5 = core_globals_get();
    if (*(char *)(iVar5 + 0x9ee) != '\0') {
      nwk_pim_stop_poll();
    }
  }
  else {
    nwk_pim_start_poll();
_L0:
    uVar1 = 0;
  }
  iVar5 = core_globals_get();
  if ((*(byte *)(iVar5 + 0xaf8) & 0x40) != 0) {
    iVar5 = core_globals_get();
    uVar6 = *(uint *)(*(int *)(iVar5 + 0xac4) + 0x14);
    uVar4 = nwk_ed_get_keepalive_interval();
    if (uVar4 / 1000 < (uVar6 & 0xfffff)) {
      uVar4 = nwk_ed_get_keepalive_interval();
      iVar5 = core_globals_get();
      uVar6 = *(uint *)(*(int *)(iVar5 + 0xac4) + 0x14);
      *(uint *)(*(int *)(iVar5 + 0xac4) + 0x14) =
           uVar6 & 0xfff00000 | (uVar6 & 0xfffff) - uVar4 / 1000 & 0xfffff;
      if (uVar1 != 0) goto _L0;
    }
    else {
      nwk_pim_stop_poll();
      uStack_24 = 9;
      uVar2 = nwk_get_parent_shortaddr();
      uVar3 = uStack_24;
      uStack_24._3_1_ = SUB41(uVar3,3);
      uStack_24._0_3_ =
           CONCAT12((char)((ushort)uVar2 >> 8),CONCAT11((char)uVar2,(undefined1)uStack_24));
      nwk_handle_network_status_local(&uStack_24);
    }
    return;
  }
_L0:
  iVar5 = core_globals_get();
  *(byte *)(iVar5 + 0xaf8) = *(byte *)(iVar5 + 0xaf8) & 0xc0;
  iVar5 = core_globals_get();
  uVar3 = nwk_ed_get_keepalive_interval();
  milli_timer_start(iVar5 + 0xae4,uVar3);
  return;
}

