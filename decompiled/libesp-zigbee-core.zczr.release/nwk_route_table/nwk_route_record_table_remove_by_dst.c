...

{
  ...
  pnVar2 = nwk_route_record_table_find(dst_addr);
  if (pnVar2 == (nwk_route_record_t *)0x0) {
    return;
  }
  iVar3 = core_globals_get();
  blk_busy = *(nwk_route_record_t **)(iVar3 + 0xcd4);
  iVar3 = core_globals_get();
  blk_busy_00 = *(bitmap_t **)(iVar3 + 0xcd8);
  iVar3 = core_globals_get();
  uVar5 = (uint)*(ushort *)(iVar3 + 0xcdc);
  iVar3 = 0x1c;
  if (pnVar2 < blk_busy) {
    register0x00002008 = (BADSPACEBASE *)auStack_10;
    unaff_retaddr = 0x10180;
    mempool_free_idx((bitmap_t *)blk_busy,(uint16_t)blk_busy_00,0x1c);
  }
