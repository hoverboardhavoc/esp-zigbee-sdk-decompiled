...

{
  ...
  if (param_1 == (undefined2 *)0x0) {
    ...
    esp_log(1,0x100c4,"E (%lu) %s: %s(%d): Invalid ota file header\n",uVar12,0x100c4,
            "zb_zcl_create_ota_upgrade_header",0x8d);
    ...
    if (puVar10 == (undefined1 *)0x0) {
      uVar12 = esp_log_timestamp();
      esp_log(1,0x100c4,"E (%lu) %s: %s(%d): No memory for OTA server notify request\n",uVar12,
              0x100c4,"zb_zcl_create_ota_upgrade_header",0x90);
    }
    else {
      *puVar10 = 0x1e;
      puVar10[1] = 0xf1;
