 ...
 * Source: libesp_zb_api.zczr.debug -> esp_zigbee_touchlink.o -> esp_zb_touchlink_send_get_group_identifiers_cmd_req
 *
...

{
  ...
  if (iVar2 == 0) {
    ...
    esp_log(1,"ESP_ZIGBEE_TOUCHLINK","E (%lu) %s: %s(%d): Not memory for buffer id\n",uVar5,
            "ESP_ZIGBEE_TOUCHLINK","esp_zb_touchlink_send_get_group_identifiers_cmd_req",0x6b);
  }
  else {
