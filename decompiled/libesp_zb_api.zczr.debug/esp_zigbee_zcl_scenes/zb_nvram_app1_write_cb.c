...

{
  ...
  if (__s == (short *)0x0) {
    ...
    esp_log(1,"ESP_ZIGBEE_ZCL_SCENES",
            "E (%lu) %s: %s(%d): No memory to write the scene table to NVRAM, size: %d\n",uVar9,
            "ESP_ZIGBEE_ZCL_SCENES","zb_nvram_app1_write_cb",0xa8,__size);
    uVar9 = 0xffffffea;
  }
  else {
    memset(__s,0,__size);
    ...
    for (iVar14 = 0; iVar14 < (int)(uint)esp_zb_zcl_scenes_table_capacity; iVar14 = iVar14 + 1) {
      ...
      if ((puVar11 != (undefined1 *)0x0) && (*(short *)(puVar11 + 2) != -1)) {
        ...
        *puVar10 = *puVar11;
        ...
        if (uVar12 != 0) {
          uVar9 = esp_log_timestamp();
          esp_log(1,"ESP_ZIGBEE_ZCL_SCENES","E (%lu) %s: %s(%d): Failed to store ZCL scene table\n",
                  uVar9,"ESP_ZIGBEE_ZCL_SCENES","zb_nvram_app1_write_cb",0xbe);
          uVar9 = 0xffffffff;
          goto _L0;
        }
      }
