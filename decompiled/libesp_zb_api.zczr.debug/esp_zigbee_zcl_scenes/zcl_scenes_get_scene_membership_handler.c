...

{
  ...
  if ((psVar3 == (short *)0x0) || (iVar4 == 0)) {
    ...
    esp_log(1,"ESP_ZIGBEE_ZCL_SCENES","E (%lu) %s: %s(%d): Invalid get_scene_membership request\n",
            uVar9,"ESP_ZIGBEE_ZCL_SCENES","zcl_scenes_get_scene_membership_handler",0x276);
    uVar9 = 0xffffffff;
  }
  else {
    ...
    if (iVar5 == 0) {
      ...
      esp_log(1,"ESP_ZIGBEE_ZCL_SCENES",
              "E (%lu) %s: %s(%d): No buffer available to respond to get_scene_membership request\n"
              ,uVar9,"ESP_ZIGBEE_ZCL_SCENES","zcl_scenes_get_scene_membership_handler",0x278);
      uVar9 = 0xffffffff;
    }
    else {
