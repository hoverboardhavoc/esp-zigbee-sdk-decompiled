...

{
  ...
  if (param_1 == 0) {
    ...
    esp_log(1,"ESP_ZIGBEE_DM_ADAPTER","E (%lu) %s: %s(%d): Uninitialized endpoint linked list!\n",
            uVar3,"ESP_ZIGBEE_DM_ADAPTER","esp_zb_internal_ep_list_to_device_ctx",0x123);
  ...
  if (puVar1 == (undefined1 *)0x0) {
    uVar3 = esp_log_timestamp();
    esp_log(1,"ESP_ZIGBEE_DM_ADAPTER","E (%lu) %s: %s(%d): No memory for device context\n",uVar3,
            "ESP_ZIGBEE_DM_ADAPTER","esp_zb_internal_ep_list_to_device_ctx",0x129);
_L0:
    ...
    *puVar1 = (char)__nmemb;
    if (__nmemb == 0) {
      ...
      if (pvVar2 == (void *)0x0) {
        uVar3 = esp_log_timestamp();
        esp_log(1,"ESP_ZIGBEE_DM_ADAPTER","E (%lu) %s: %s(%d): No memory for endpoint desc list\n",
                uVar3,"ESP_ZIGBEE_DM_ADAPTER","esp_zb_internal_ep_list_to_device_ctx",0x12d);
        goto _L0;
      }
      ...
      for (iVar4 = 0; iVar4 < (int)__nmemb; iVar4 = iVar4 + 1) {
        ...
        *(void **)(*(int *)(puVar1 + 1) + iVar4 * 4) = __dest;
        if (__dest == (void *)0x0) {
          uVar3 = esp_log_timestamp();
          esp_log(1,"ESP_ZIGBEE_DM_ADAPTER","E (%lu) %s: %s(%d): No memory for endpoint desc\n",
                  uVar3,"ESP_ZIGBEE_DM_ADAPTER","esp_zb_internal_ep_list_to_device_ctx",0x132);
          goto _L0;
        }
