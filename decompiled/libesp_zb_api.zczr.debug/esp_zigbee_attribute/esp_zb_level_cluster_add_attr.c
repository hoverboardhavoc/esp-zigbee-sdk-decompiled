/*
 * Last changed at upstream commit 0bff9367811bb8cb2d99200afddf6ceefe2628f8
 * https://github.com/espressif/esp-zigbee-sdk/commit/0bff9367811bb8cb2d99200afddf6ceefe2628f8
 * Upstream date: 2024-12-06 13:11:49 +0800
 * Upstream subject: esp-zigbee-sdk: (e9475ff2)
 * Source: libesp_zb_api.zczr.debug -> esp_zigbee_attribute.o -> esp_zb_level_cluster_add_attr
 *
 * (C) Espressif, Apache License 2.0.
 * Derivative work (this file): mechanical decompile via Ghidra (NSA, Apache 2.0).
 * Decompiler output may be incomplete or differ from original semantics.
 */

undefined4 esp_zb_level_cluster_add_attr(uint param_1,undefined4 param_2)

{
  undefined4 uVar1;
  undefined4 uVar2;
  
  if (param_1 == 0xf) {
    uVar2 = 3;
    uVar1 = 0x18;
  }
  else if (param_1 < 0x10) {
    if (param_1 == 3) {
      uVar2 = 1;
      uVar1 = 0x20;
    }
    else if (param_1 < 4) {
      if (param_1 == 1) {
        uVar2 = 1;
        uVar1 = 0x21;
      }
      else if (param_1 == 2) {
        uVar2 = 1;
        uVar1 = 0x20;
      }
      else {
        if (param_1 != 0) {
_L0:
          uVar1 = esp_log_timestamp(param_2);
          esp_log_write(1,0x10000,&_LC11,uVar1,0x10000);
          return 0x102;
        }
        uVar2 = 0x15;
        uVar1 = 0x20;
      }
    }
    else if (param_1 == 5) {
      uVar2 = 1;
      uVar1 = 0x21;
    }
    else if (param_1 == 6) {
      uVar2 = 1;
      uVar1 = 0x21;
    }
    else {
      if (param_1 != 4) goto _L0;
      uVar2 = 0x15;
      uVar1 = 0x21;
    }
  }
  else if (param_1 == 0x13) {
    uVar2 = 3;
    uVar1 = 0x21;
  }
  else if (param_1 < 0x14) {
    if (param_1 == 0x11) {
      uVar2 = 3;
      uVar1 = 0x20;
    }
    else if (param_1 == 0x12) {
      uVar2 = 3;
      uVar1 = 0x21;
    }
    else {
      if (param_1 != 0x10) goto _L0;
      uVar2 = 3;
      uVar1 = 0x21;
    }
  }
  else if (param_1 == 0x4000) {
    uVar2 = 3;
    uVar1 = 0x20;
  }
  else if (param_1 == 0xefff) {
    uVar2 = 0x40;
    uVar1 = 0x12;
  }
  else {
    if (param_1 != 0x14) goto _L0;
    uVar2 = 3;
    uVar1 = 0x20;
  }
  uVar1 = esp_zb_internal_cluster_add_attr(8,param_1,uVar1,uVar2,0xffff);
  return uVar1;
}

