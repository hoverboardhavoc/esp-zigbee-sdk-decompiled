 ...
 * Source: libesp_zb_api.zczr.debug -> esp_zigbee_zcl_command.o -> esp_zb_zcl_price_get_current_price_cmd_req
 *
...

{
  ...
  if (iVar1 == 0) {
    ...
    esp_log(1,"ESP_ZIGBEE_COMMAND","E (%lu) %s: %s(%d): No I/O buffer for %s\n",uVar2,
            "ESP_ZIGBEE_COMMAND","esp_zb_zcl_price_get_current_price_cmd_req",0x7c6,
