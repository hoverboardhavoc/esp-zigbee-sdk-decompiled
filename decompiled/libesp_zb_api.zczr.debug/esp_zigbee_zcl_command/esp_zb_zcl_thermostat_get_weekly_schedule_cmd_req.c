 ...
 * Source: libesp_zb_api.zczr.debug -> esp_zigbee_zcl_command.o -> esp_zb_zcl_thermostat_get_weekly_schedule_cmd_req
 *
...

{
  ...
  if (iVar1 == 0) {
    ...
    esp_log(1,"ESP_ZIGBEE_COMMAND","E (%lu) %s: %s(%d): No I/O buffer for %s\n",uVar3,
            "ESP_ZIGBEE_COMMAND","esp_zb_zcl_thermostat_get_weekly_schedule_cmd_req",0x74c,
            "esp_zb_zcl_thermostat_get_weekly_schedule_cmd_req");
    pcVar4 = (char *)zb_zcl_get_ctx();
    *pcVar4 = *pcVar4 + '\x01';
  }
  else {
    puVar2 = (undefined1 *)zb_zcl_start_command_header(0x11,0,2,0);
