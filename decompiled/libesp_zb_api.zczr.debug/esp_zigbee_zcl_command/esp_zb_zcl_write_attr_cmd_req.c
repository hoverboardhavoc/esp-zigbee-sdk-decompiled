...

{
  ...
  while ((iVar1 < (int)(uint)*(byte *)(param_1 + 0x16) && (*(int *)(param_1 + 0x18) != 0))) {
    ...
    if (iVar6 == 0xffff) {
      ...
      esp_log(1,"ESP_ZIGBEE_COMMAND",
              "E (%lu) %s: %s(%d): Error attribute field, id: 0x%x, type: 0x%x, size: %d\n",uVar2,
              "ESP_ZIGBEE_COMMAND","esp_zb_zcl_write_attr_cmd_req",0x83,
  ...
  if (iVar1 == 0) {
    ...
    esp_log(1,"ESP_ZIGBEE_COMMAND","E (%lu) %s: %s(%d): No I/O buffer for %s\n",uVar2,
            "ESP_ZIGBEE_COMMAND","esp_zb_zcl_write_attr_cmd_req",0x89,
