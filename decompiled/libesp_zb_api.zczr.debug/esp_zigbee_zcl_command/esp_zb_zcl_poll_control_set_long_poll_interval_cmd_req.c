...

{
  ...
  if (param_1 == 0) {
    ...
    esp_log(1,"ESP_ZIGBEE_COMMAND","E (%lu) %s: %s(%d): Invalid argument\n",uVar3,
            "ESP_ZIGBEE_COMMAND","esp_zb_zcl_poll_control_set_long_poll_interval_cmd_req",0x896);
    ...
    *pcVar4 = *pcVar4 + '\x01';
    ...
    if (iVar1 == 0) {
      ...
      esp_log(1,"ESP_ZIGBEE_COMMAND","E (%lu) %s: %s(%d): No I/O buffer for %s\n",uVar3,
              "ESP_ZIGBEE_COMMAND","esp_zb_zcl_poll_control_set_long_poll_interval_cmd_req",0x897,
