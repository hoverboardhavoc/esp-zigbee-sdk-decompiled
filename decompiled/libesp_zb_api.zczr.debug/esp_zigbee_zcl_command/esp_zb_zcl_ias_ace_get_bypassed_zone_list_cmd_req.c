 ...
 * Source: libesp_zb_api.zczr.debug -> esp_zigbee_zcl_command.o -> esp_zb_zcl_ias_ace_get_bypassed_zone_list_cmd_req
 *
...
void esp_zb_zcl_ias_ace_get_bypassed_zone_list_cmd_req(int param_1)

{
  ...
  if (iVar1 == 0) {
    ...
    esp_log(1,"ESP_ZIGBEE_COMMAND","E (%lu) %s: %s(%d): No I/O buffer for %s\n",uVar2,
            "ESP_ZIGBEE_COMMAND","esp_zb_zcl_ias_ace_get_bypassed_zone_list_cmd_req",0x645,
            "esp_zb_zcl_ias_ace_get_bypassed_zone_list_cmd_req");
    pcVar3 = (char *)zb_zcl_get_ctx();
    *pcVar3 = *pcVar3 + '\x01';
  }
  else {
    uVar2 = zb_zcl_start_command_header(1,0,8,0);
