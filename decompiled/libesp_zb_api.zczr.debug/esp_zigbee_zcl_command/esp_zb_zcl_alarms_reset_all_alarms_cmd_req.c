 ...
 * Source: libesp_zb_api.zczr.debug -> esp_zigbee_zcl_command.o -> esp_zb_zcl_alarms_reset_all_alarms_cmd_req
 *
...
void esp_zb_zcl_alarms_reset_all_alarms_cmd_req(int param_1)

{
  ...
  if (param_1 == 0) {
    ...
    esp_log(1,"ESP_ZIGBEE_COMMAND","E (%lu) %s: %s(%d): Invalid argument\n",uVar3,
            "ESP_ZIGBEE_COMMAND","esp_zb_zcl_alarms_reset_all_alarms_cmd_req",0x8cf);
    ...
    *pcVar4 = *pcVar4 + '\x01';
    ...
    if (iVar1 == 0) {
      ...
      esp_log(1,"ESP_ZIGBEE_COMMAND","E (%lu) %s: %s(%d): No I/O buffer for %s\n",uVar3,
              "ESP_ZIGBEE_COMMAND","esp_zb_zcl_alarms_reset_all_alarms_cmd_req",0x8d0,
