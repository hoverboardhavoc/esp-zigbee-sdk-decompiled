 ...
 * Source: libesp_zb_api.zczr.debug -> esp_zigbee_zcl_command.o -> esp_zb_zcl_custom_cluster_handlers_update
 *
...

{
  ...
  if ((iVar4 == 0) || (iVar5 = *(int *)(param_1 + 4), iVar5 == 0)) {
    ...
    esp_log(1,"ESP_ZIGBEE_COMMAND","E (%lu) %s: %s(%d): Invalid argument\n",uVar3,
            "ESP_ZIGBEE_COMMAND","esp_zb_zcl_custom_cluster_handlers_update",0x996);
    uVar3 = 0x102;
  }
  else {
    ...
    if (*(byte *)(iVar2 + 0x2dc) < 0x1e) {
      ...
      uVar3 = esp_log_timestamp();
      esp_log(1,"ESP_ZIGBEE_COMMAND","E (%lu) %s: %s(%d): No memory for ZCL handlers\n",uVar3,
              "ESP_ZIGBEE_COMMAND","esp_zb_zcl_custom_cluster_handlers_update",0x997);
      uVar3 = 0x101;
    }
  }
  return uVar3;
