  ...
  if (iVar3 == 0) {
    ...
    while (psVar2 != (short *)0x0) {
      if (*psVar2 == sVar1) {
        ...
        esp_log_write(1,"ESP_ZIGBEE_CLUSTER",&_LC21,uVar5,"ESP_ZIGBEE_CLUSTER",
                      "esp_zb_cluster_list_update_on_off_switch_config_cluster",0x94d,sVar1);
        return 0x101;
      }
    ...
    if (iVar3 != 0) {
      ...
      esp_log_write(1,"ESP_ZIGBEE_CLUSTER",&_LC22,uVar5,"ESP_ZIGBEE_CLUSTER",
                    "esp_zb_cluster_list_update_on_off_switch_config_cluster",0x967,sVar1);
    }
  }
  else {
    uVar5 = esp_log_timestamp();
    esp_log_write(1,"ESP_ZIGBEE_CLUSTER",&_LC20,uVar5,"ESP_ZIGBEE_CLUSTER",
                  "esp_zb_cluster_list_update_on_off_switch_config_cluster",0x947);
  }
  return iVar3;
}
