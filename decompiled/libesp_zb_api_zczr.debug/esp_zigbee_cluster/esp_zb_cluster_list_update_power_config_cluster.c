  ...
  if (iVar3 == 0) {
    ...
    while (psVar2 != (short *)0x0) {
      if (*psVar2 == sVar1) {
        ...
        esp_log_write(1,"ESP_ZIGBEE_CLUSTER",&_LC25,uVar5,"ESP_ZIGBEE_CLUSTER",
                      "esp_zb_cluster_list_update_power_config_cluster",0x90f,sVar1);
        return 0x101;
      }
    ...
    if (iVar3 != 0) {
      ...
      esp_log_write(1,"ESP_ZIGBEE_CLUSTER",&_LC26,uVar5,"ESP_ZIGBEE_CLUSTER",
                    "esp_zb_cluster_list_update_power_config_cluster",0x929,sVar1);
    }
  }
  else {
    uVar5 = esp_log_timestamp();
    esp_log_write(1,"ESP_ZIGBEE_CLUSTER",&_L0,uVar5,"ESP_ZIGBEE_CLUSTER",
                  "esp_zb_cluster_list_update_power_config_cluster",0x909);
  }
  return iVar3;
}
