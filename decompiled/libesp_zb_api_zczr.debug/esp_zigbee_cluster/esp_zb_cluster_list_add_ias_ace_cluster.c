 ...
 * Source: libesp_zb_api_zczr.debug -> esp_zigbee_cluster.o -> esp_zb_cluster_list_add_ias_ace_cluster
 *
...

{
  ...
  aiStack_24[0] = 0;
  iVar2 = esp_zb_cluster_list_get_tail(param_1,param_2,0x501,param_3,aiStack_24);
  iVar1 = aiStack_24[0];
  if (aiStack_24[0] == 0) {
    uVar3 = zcl_convert_attr_list_to_array(param_2);
    uStack_3c = CONCAT22(extraout_a0,0x501);
    if (param_3 == 1) {
      puVar4 = &esp_zb_zcl_ias_ace_init_server;
    }
    else if (param_3 == 2) {
      puVar4 = &esp_zb_zcl_ias_ace_init_client;
    }
    else {
