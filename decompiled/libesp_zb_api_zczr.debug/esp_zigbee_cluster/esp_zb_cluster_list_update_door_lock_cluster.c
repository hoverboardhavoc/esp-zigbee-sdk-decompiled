 ...
 * Source: libesp_zb_api_zczr.debug -> esp_zigbee_cluster.o -> esp_zb_cluster_list_update_door_lock_cluster
 *
...
int esp_zb_cluster_list_update_door_lock_cluster(int param_1,int param_2,int param_3)

{
  ...
  iVar3 = esp_zb_cluster_list_check(0x101);
  if (iVar3 == 0) {
    ...
    while (psVar2 != (short *)0x0) {
      if (*psVar2 == sVar1) {
        ...
        if (puVar4 != (undefined4 *)0x0) {
          ...
          uStack_38 = CONCAT22(extraout_a0,0x101);
          if (param_3 == 1) {
            puVar6 = &esp_zb_zcl_door_lock_init_server;
          }
          else if (param_3 == 2) {
            puVar6 = &esp_zb_zcl_door_lock_init_client;
          }
          else {
        ...
        esp_log_write(1,"ESP_ZIGBEE_CLUSTER",&_LC24,uVar5,"ESP_ZIGBEE_CLUSTER",
                      "esp_zb_cluster_list_update_door_lock_cluster",0xb34,sVar1);
        return 0x101;
      }
    ...
    if (iVar3 != 0) {
      ...
      esp_log_write(1,"ESP_ZIGBEE_CLUSTER",&_LC25,uVar5,"ESP_ZIGBEE_CLUSTER",
                    "esp_zb_cluster_list_update_door_lock_cluster",0xb4e,sVar1);
    }
  }
  else {
    uVar5 = esp_log_timestamp();
    esp_log_write(1,"ESP_ZIGBEE_CLUSTER",&_L0,uVar5,"ESP_ZIGBEE_CLUSTER",
                  "esp_zb_cluster_list_update_door_lock_cluster",0xb2e);
  }
  return iVar3;
}
