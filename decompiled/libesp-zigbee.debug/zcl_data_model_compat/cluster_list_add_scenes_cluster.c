 ...
 * Source: libesp-zigbee.debug -> zcl_data_model_compat.o -> cluster_list_add_scenes_cluster
 *
...
esp_err_t cluster_list_add_scenes_cluster
...

{
  ...
  if (cluster_list == (esp_zb_cluster_list_t *)0x0) {
    ...
    if ((role_mask & 1) == 0) {
      puVar3 = &ezb_zcl_scenes_cluster_client_init;
    }
    else {
      puVar3 = &ezb_zcl_scenes_cluster_server_init;
    }
    ((zcl_cluster_desc_t *)attr_list)->cluster_init = puVar3;
    zcl_cluster_desc_add_revision_attr((zcl_cluster_desc_t *)attr_list,3);
    if ((role_mask & 1) != 0) {
      uStack_11 = 0xf;
      ezb_zcl_cluster_desc_add_manuf_attr(attr_list,0xeff0,0x20,0x40,0x131b,&uStack_11);
