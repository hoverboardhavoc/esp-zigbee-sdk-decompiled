 ...
 * Source: libesp-zigbee.debug -> zdo_cmd_compat.o -> esp_zb_zdo_permit_join_rsp_wrapper
 *
...
void esp_zb_zdo_permit_join_rsp_wrapper
...

{
  ...
  if (pcVar2 != (code *)0x0) {
    pvVar1 = user_ctx;
    if (result->error == 0) {
      if (result->rsp != (ezb_zdp_nwk_mgmt_permit_joining_rsp_field_t *)0x0) {
      ...
      pvVar1 = (void *)__assert_func("//build/esp-zigbee/src/platform_esp/esp-zigbee/compat/src/zdo_cmd_compat.c"
                                     ,0x18b,"esp_zb_zdo_permit_join_rsp_wrapper","result->rsp");
    }
