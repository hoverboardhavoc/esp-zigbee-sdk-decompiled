 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_color_move_color_cmd_req
 *
...

{
  ...
  ezb_zcl_color_control_move_color_cmd_t req;
  ...
  req.payload.rate_x = 0;
  req.payload.rate_y = 0;
  if (cmd_req == (esp_zb_zcl_color_move_color_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument","esp_zb_zcl_color_move_color_cmd_req",
            0x3fb);
    uVar1 = 0xff;
  }
  else {
    ...
    req.cmd_ctrl.cnf_ctx.user_ctx = *(void **)&cmd_req->rate_x;
    iVar2 = ezb_zcl_color_control_move_color_cmd_req(local_30);
    if (iVar2 == 0) {
