 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_price_get_tier_labels_cmd_req
 *
...

{
  ...
  ezb_zcl_price_get_tier_labels_cmd_t req;
  ...
  if (cmd_req == (esp_zb_zcl_price_get_tier_labels_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument",
            "esp_zb_zcl_price_get_tier_labels_cmd_req",0x5a9);
    uVar1 = 0xff;
  }
  else {
    ...
    ;
    req.cmd_ctrl.cnf_ctx.user_ctx = (void *)cmd_req->issuer_tariff_id;
    iVar2 = ezb_zcl_price_get_tier_labels_cmd_req((ezb_zcl_cluster_cmd_ctrl_t *)auStack_2c);
    if (iVar2 == 0) {
