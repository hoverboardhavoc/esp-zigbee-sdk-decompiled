 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_groups_get_group_membership_cmd_req
 *
...
uint8_t esp_zb_zcl_groups_get_group_membership_cmd_req
...

{
  ...
  ezb_zcl_groups_get_group_membership_cmd_t req;
  ...
  req.payload.group_count = '\0';
  req.payload._1_3_ = 0;
  if (cmd_req == (esp_zb_zcl_groups_get_group_membership_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument",
            "esp_zb_zcl_groups_get_group_membership_cmd_req",0x2f7);
    uVar1 = 0xff;
  }
  else {
    ...
    req.cmd_ctrl.cnf_ctx.user_ctx =
         (void *)CONCAT31(req.cmd_ctrl.cnf_ctx.user_ctx._1_3_,cmd_req->group_count);
    req.payload._0_4_ = cmd_req->group_list;
    iVar2 = ezb_zcl_groups_get_group_membership_cmd_req(local_30);
    if (iVar2 == 0) {
