 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_color_move_saturation_cmd_req
 *
...

{
  ...
  ezb_zcl_color_control_move_saturation_cmd_t req;
  ...
  if (cmd_req == (esp_zb_zcl_color_move_saturation_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument",
            "esp_zb_zcl_color_move_saturation_cmd_req",0x3bc);
    uVar2 = 0xff;
  }
  else {
    ...
    ;
    uVar1._0_1_ = cmd_req->move_mode;
    uVar1._1_1_ = cmd_req->rate;
    req.cmd_ctrl.cnf_ctx.user_ctx = (void *)CONCAT22(req.cmd_ctrl.cnf_ctx.user_ctx._2_2_,uVar1);
    iVar3 = ezb_zcl_color_control_move_saturation_cmd_req((ezb_zcl_cluster_cmd_ctrl_t *)auStack_2c);
    if (iVar3 == 0) {
