 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_metering_get_profile_cmd_req
 *
...

{
  ...
  ezb_zcl_metering_get_profile_cmd_t req;
  ...
  req.payload.interval_channel = '\0';
  req.payload._1_3_ = 0;
  req.payload.end_time = 0;
  if (cmd_req == (esp_zb_metering_get_profile_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument",
            "esp_zb_zcl_metering_get_profile_cmd_req",0x54a);
    uVar1 = 0xff;
  }
  else {
    ...
    req.cmd_ctrl.cnf_ctx.user_ctx =
         (void *)CONCAT31(req.cmd_ctrl.cnf_ctx.user_ctx._1_3_,cmd_req->interval_channel);
    req.payload._0_4_ = cmd_req->end_time;
    req.payload.end_time = CONCAT31(req.payload.end_time._1_3_,cmd_req->number_of_periods);
    iVar2 = ezb_zcl_metering_get_profile_cmd_req((ezb_zcl_cluster_cmd_ctrl_t *)auStack_34);
    if (iVar2 == 0) {
