 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_color_step_color_cmd_req
 *
...

{
  ...
  ezb_zcl_color_control_step_color_cmd_t req;
  ...
  req.payload.step_x = '\0';
  req.payload.step_y = '\0';
  req.payload.transition_time = 0;
  if (cmd_req == (esp_zb_zcl_color_step_color_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument","esp_zb_zcl_color_step_color_cmd_req",
            0x40a);
    uVar1 = 0xff;
  }
  else {
    ...
    req.cmd_ctrl.cnf_ctx.user_ctx =
         (void *)CONCAT22(cmd_req->transition_time,
                          CONCAT11((char)cmd_req->step_y,(char)cmd_req->step_x));
    iVar2 = ezb_zcl_color_control_step_color_cmd_req(local_30);
    if (iVar2 == 0) {
