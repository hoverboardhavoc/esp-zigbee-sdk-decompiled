 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_report_attr_cmd_req
 *
...

{
  ...
  if (cmd_req == (esp_zb_zcl_report_attr_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument","esp_zb_zcl_report_attr_cmd_req",0xce);
    eVar1 = 0x102;
  }
  else {
    ...
    req.cmd_ctrl.cnf_ctx.user_ctx =
         (void *)CONCAT22(req.cmd_ctrl.cnf_ctx.user_ctx._2_2_,cmd_req->attributeID);
    ezb_zcl_report_attr_cmd_req(local_30);
