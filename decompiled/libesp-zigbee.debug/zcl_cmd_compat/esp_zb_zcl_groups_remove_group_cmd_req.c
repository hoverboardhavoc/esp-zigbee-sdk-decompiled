 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_groups_remove_group_cmd_req
 *
...

{
  ...
  ezb_zcl_groups_remove_group_cmd_t req;
  ...
  if (cmd_req == (esp_zb_zcl_groups_add_group_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument",
            "esp_zb_zcl_groups_remove_group_cmd_req",0x2cf);
    uVar1 = 0xff;
  }
  else {
    ...
    iVar2 = ezb_zcl_groups_remove_cmd_req((ezb_zcl_cluster_cmd_ctrl_t *)auStack_2c);
    if (iVar2 == 0) {
