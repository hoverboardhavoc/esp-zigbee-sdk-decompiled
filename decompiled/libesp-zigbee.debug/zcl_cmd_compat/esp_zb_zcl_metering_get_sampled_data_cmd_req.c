 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_metering_get_sampled_data_cmd_req
 *
...

{
  ...
  req.payload.sample_id = 0;
  req.payload._2_2_ = 0;
  req.payload.earliest_start_time = 0;
  if (cmd_req == (esp_zb_metering_get_sampled_data_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument",
            "esp_zb_zcl_metering_get_sampled_data_cmd_req",0x57a);
    uVar1 = 0xff;
  }
  else {
    ...
    req.cmd_ctrl.cnf_ctx.user_ctx =
         (void *)CONCAT22(req.cmd_ctrl.cnf_ctx.user_ctx._2_2_,cmd_req->sample_id);
    req.payload._0_4_ = cmd_req->earliest_sample_time;
    req.payload.earliest_start_time =
         CONCAT31(req.payload.earliest_start_time._1_3_,cmd_req->sample_type);
    req.payload.earliest_start_time =
         CONCAT22(cmd_req->number_of_samples,(undefined2)req.payload.earliest_start_time);
    iVar2 = ezb_zcl_metering_get_sampled_data_cmd_req((ezb_zcl_cluster_cmd_ctrl_t *)auStack_34);
    if (iVar2 == 0) {
