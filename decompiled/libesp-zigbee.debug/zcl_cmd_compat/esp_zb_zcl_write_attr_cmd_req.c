 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_write_attr_cmd_req
 *
...

{
  ...
  req.payload.attr_number = '\0';
  req.payload._1_3_ = 0;
  if (cmd_req == (esp_zb_zcl_write_attr_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument","esp_zb_zcl_write_attr_cmd_req",0xb2);
    uVar1 = 0xff;
  }
  else {
    ...
    req.cmd_ctrl.cnf_ctx.user_ctx =
         (void *)CONCAT31(req.cmd_ctrl.cnf_ctx.user_ctx._1_3_,cmd_req->attr_number);
    req.payload._0_4_ = cmd_req->attr_field;
    iVar2 = ezb_zcl_write_attr_cmd_req((ezb_zcl_cmd_ctrl_t *)auStack_34);
    if (iVar2 == 0) {
