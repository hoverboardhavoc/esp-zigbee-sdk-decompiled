 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_color_move_to_color_cmd_req
 *
...

{
  ...
  ezb_zcl_color_control_move_to_color_cmd_t req;
  ...
  req.payload.color_x = 0;
  req.payload.color_y = 0;
  if (cmd_req == (esp_zb_zcl_color_move_to_color_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument",
            "esp_zb_zcl_color_move_to_color_cmd_req",0x3eb);
    uVar1 = 0xff;
  }
  else {
    convert_to_ezb_specific_cmd_ctrl
              ((ezb_zcl_cluster_cmd_ctrl_t *)local_30,(esp_zb_zcl_specific_cmd_header_t *)cmd_req);
    req.cmd_ctrl.cnf_ctx.user_ctx = *(void **)&cmd_req->color_x;
    req.payload.color_x = cmd_req->transition_time;
    iVar2 = ezb_zcl_color_control_move_to_color_cmd_req(local_30);
    if (iVar2 == 0) {
