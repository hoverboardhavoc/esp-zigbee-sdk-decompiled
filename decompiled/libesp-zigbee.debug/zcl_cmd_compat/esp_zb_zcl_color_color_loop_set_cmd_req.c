 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_color_color_loop_set_cmd_req
 *
...

{
  ...
  ezb_zcl_color_control_color_loop_set_cmd_t req;
  ...
  req.payload.update_flags = '\0';
  req.payload.action = '\0';
  req.payload.direction = '\0';
  req.payload._3_1_ = 0;
  req.payload.time = 0;
  req.payload.start_hue = 0;
  if (cmd_req == (esp_zb_zcl_color_color_loop_set_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument",
            "esp_zb_zcl_color_color_loop_set_cmd_req",0x475);
    uVar2 = 0xff;
  }
  else {
    ...
    ;
    uVar1._0_1_ = cmd_req->update_flags;
    uVar1._1_1_ = cmd_req->action;
    uVar1._2_1_ = cmd_req->direction;
    req.cmd_ctrl.cnf_ctx.user_ctx = (void *)CONCAT13(req.cmd_ctrl.cnf_ctx.user_ctx._3_1_,uVar1);
    req.payload._0_4_ = *(undefined4 *)&cmd_req->time;
    iVar3 = ezb_zcl_color_control_color_loop_set_cmd_req((ezb_zcl_cluster_cmd_ctrl_t *)auStack_34);
    if (iVar3 == 0) {
