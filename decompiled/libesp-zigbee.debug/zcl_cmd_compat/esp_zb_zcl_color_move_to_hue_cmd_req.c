 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_color_move_to_hue_cmd_req
 *
...

{
  ...
  ezb_zcl_color_control_move_to_hue_cmd_t req;
  ...
  if (cmd_req == (esp_zb_zcl_color_move_to_hue_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument","esp_zb_zcl_color_move_to_hue_cmd_req",
            0x37e);
    uVar2 = 0xff;
  }
  else {
    ...
    uVar1._1_1_ = cmd_req->direction;
    req.cmd_ctrl.cnf_ctx.user_ctx = (void *)CONCAT22(cmd_req->transition_time,uVar1);
    iVar3 = ezb_zcl_color_control_move_to_hue_cmd_req(local_30);
    if (iVar3 == 0) {
