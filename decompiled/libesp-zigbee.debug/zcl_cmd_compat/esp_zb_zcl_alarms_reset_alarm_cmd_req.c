 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_alarms_reset_alarm_cmd_req
 *
...

{
  ...
  ezb_zcl_alarms_reset_alarm_cmd_t req;
  ...
  if (cmd_req == (esp_zb_zcl_alarms_reset_alarm_cmd_req_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument","esp_zb_zcl_alarms_reset_alarm_cmd_req"
            ,0x5b8);
    uVar1 = 0xff;
  }
  else {
    ...
    req.cmd_ctrl.cnf_ctx.user_ctx =
         (void *)CONCAT31(req.cmd_ctrl.cnf_ctx.user_ctx._1_3_,cmd_req->alarm_code);
    req.cmd_ctrl.cnf_ctx.user_ctx =
         (void *)CONCAT22(cmd_req->cluster_id,req.cmd_ctrl.cnf_ctx.user_ctx._0_2_);
    iVar2 = ezb_zcl_alarms_reset_alarm_cmd_req((ezb_zcl_cluster_cmd_ctrl_t *)auStack_2c);
    if (iVar2 == 0) {
