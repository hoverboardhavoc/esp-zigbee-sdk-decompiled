 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_level_step_with_onoff_cmd_req
 *
...

{
  ...
  if (cmd_req == (esp_zb_zcl_level_step_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument",
            "esp_zb_zcl_level_step_with_onoff_cmd_req",0x2a4);
    uVar2 = 0xff;
  }
  else {
    ...
    iVar3 = ezb_zcl_level_step_with_on_off_cmd_req(local_30);
    if (iVar3 == 0) {
