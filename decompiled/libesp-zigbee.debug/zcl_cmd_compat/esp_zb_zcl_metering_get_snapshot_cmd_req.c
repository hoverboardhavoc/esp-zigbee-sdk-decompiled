 ...
 * Source: libesp-zigbee.debug -> zcl_cmd_compat.o -> esp_zb_zcl_metering_get_snapshot_cmd_req
 *
...

{
  ...
  req.payload.latest_end_time = 0;
  req.payload.snapshot_offset = '\0';
  req.payload._9_3_ = 0;
  if (cmd_req == (esp_zb_metering_get_snapshot_cmd_t *)0x0) {
    esp_log(0x11,"ZCL_CMD_COMPAT","%s(%d): Invalid Argument",
            "esp_zb_zcl_metering_get_snapshot_cmd_req",0x569);
    uVar1 = 0xff;
  }
  else {
    ...
    req.payload.latest_end_time =
         CONCAT31(req.payload.latest_end_time._1_3_,cmd_req->snapshot_offset);
    req.payload._8_4_ = cmd_req->snapshot_cause;
    iVar2 = ezb_zcl_metering_get_snapshot_cmd_req((ezb_zcl_cluster_cmd_ctrl_t *)auStack_38);
    if (iVar2 == 0) {
