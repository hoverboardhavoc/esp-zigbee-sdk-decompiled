  ...
  if (param_1 - 7U < 2) {
    ...
    if ((param_1 == 7) || (bVar1)) {
      if (uVar3 == 0xffffffff) {
        ...
        cVar2 = parse_address(*param_2,uVar3 * 0x3c + 0x1141c,1);
        (&DAT_00011424)[uVar3 * 0x3c] = cVar2;
        if (cVar2 == '\0') {
          uVar6 = esp_log_timestamp();
          esp_log_write(1,&_L0,&_LC13,uVar6,&_L0);
        }
        else {
          iVar4 = sscan_uint8(param_2[1],&DAT_00011425 + uVar3 * 0x3c);
          if (iVar4 == 0) {
            uVar6 = esp_log_timestamp();
            esp_log_write(1,&_L0,&_LC14,uVar6,&_L0);
          }
          else {
            iVar4 = parse_hex_u16(param_2[2],&DAT_00011428 + uVar3 * 0x3c);
            if (iVar4 == 0) {
              uVar6 = esp_log_timestamp();
              esp_log_write(1,&_L0,&_LC15,uVar6,&_L0);
            }
            else {
              iVar4 = parse_hex_u16(param_2[3],&DAT_00011426 + uVar3 * 0x3c);
              if (iVar4 == 0) {
                uVar6 = esp_log_timestamp();
                esp_log_write(1,&_L0,&_LC16,uVar6,&_L0);
              }
              else {
                iVar4 = parse_hex_u16(param_2[4],&DAT_0001142a + uVar3 * 0x3c);
                if (iVar4 == 0) {
                  uVar6 = esp_log_timestamp();
                  esp_log_write(1,&_L0,&_LC17,uVar6,&_L0);
                }
                else {
                  iVar4 = parse_hex_u8(param_2[5],&DAT_0001142c + uVar3 * 0x3c);
                  if (iVar4 == 0) {
                    uVar6 = esp_log_timestamp();
                    ...
                    __s = (char *)param_2[6];
                    sVar5 = strlen(__s);
                    if ((&DAT_0001142c)[uVar3 * 0x3c] == 'B') {
                      (&DAT_0001142d)[uVar3 * 0x3c] = (char)sVar5;
                      strncpy((char *)(uVar3 * 0x3c + 0x1142e),(char *)param_2[6],0x1f);
                    }
                    else {
                      iVar4 = parse_hex_str(__s,&DAT_0001142d + uVar3 * 0x3c,0x20,1);
                      if (iVar4 == 0) {
                        uVar6 = esp_log_timestamp();
                    ...
                    if (bVar1) {
                      (&DAT_0001144d)[uVar3 * 0x3c] = 1;
                    }
                    else {
                      (&DAT_0001144d)[uVar3 * 0x3c] = 0;
                    }
                    *(undefined4 *)(&DAT_00011418 + uVar3 * 0x3c) = 1;
