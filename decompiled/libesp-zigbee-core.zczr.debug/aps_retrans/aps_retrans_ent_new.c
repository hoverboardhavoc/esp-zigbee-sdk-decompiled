...
aps_retrans_ent_t * aps_retrans_ent_new(aps_tx_context_t *ctx)

{
  ...
  if (uVar2 < 0x20) {
    ...
    if (!_Var1) {
      memset(ctx->ents + uVar2,0,0x20);
      aps_retrans_start_fast_poll();
      return ctx->ents + uVar2;
    }
    __assert_func("//build/esp-zigbee/src/core/aps/aps_retrans.c",0x8e,"aps_retrans_ent_new",
