...

{
  ...
  if (cmd == (ezb_zcl_alarms_alarm_cmd_t *)0x0) {
    ...
    if (iVar2 == 0) {
      eVar1 = zcl_cmd_to_packet(&uStack_48,0,1,(cmd->cmd_ctrl).dis_default_rsp,0,0,9);
      if (eVar1 == 0) {
        uStack_4a = CONCAT11(uStack_4a._1_1_,(cmd->payload).alarm_code);
        eVar1 = zmsg_append_bytes(packet._32_4_,1,&uStack_4a);
        if (eVar1 == 0) {
          uStack_4a = (cmd->payload).cluster_id;
          eVar1 = zmsg_append_bytes(packet._32_4_,2,&uStack_4a);
          if (eVar1 == 0) {
            zcl_packet_send(&uStack_48,&(cmd->cmd_ctrl).cnf_ctx);
