 ...
 * Source: libesp-zigbee-core.zczr.debug -> custom.o -> ezb_zcl_custom_cmd_req_with_cluster
 *
...
ezb_err_t ezb_zcl_custom_cmd_req_with_cluster(ezb_zcl_custom_cmd_t *cmd_req,_Bool check)

{
  ...
  packet._34_2_ = 0;
  if (cmd_req == (ezb_zcl_custom_cmd_t *)0x0) {
    eVar1 = 2;
  }
  else {
    ...
    if (iVar2 == 0) {
      ...
      eVar1 = zcl_cmd_to_packet(&uStack_38,1,uVar3 & 1,uVar3 >> 1 & 1,uVar3 >> 2 & 1,
                                (cmd_req->cmd_ctrl).manuf_code,cmd_req->cmd_id,
                                (cmd_req->cmd_ctrl).cluster_id);
      if ((eVar1 == 0) &&
         (eVar1 = zmsg_append_bytes(packet._32_4_,cmd_req->data_length,cmd_req->data), eVar1 == 0))
      {
