...

{
  ...
  if (*(char *)(iVar1 + 1) == '\x02') {
    if (error == 0) {
      ...
      __assert_func("//build/esp-zigbee/src/core/commissioning/bdb/bdb_touchlink.c",0xda,
                    "touchlink_target_process_discovery_done",&_LC3);
    }
    touchlink_commissioning_task_result(1);
  }
  else {
    __assert_func("//build/esp-zigbee/src/core/commissioning/bdb/bdb_touchlink.c",0xcf,
