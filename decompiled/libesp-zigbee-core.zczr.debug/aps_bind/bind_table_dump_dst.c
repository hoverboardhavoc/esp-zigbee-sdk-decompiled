...

{
  ...
  if ((dst->field_0x5 & 1) != 0) {
    log_write(3,"aps_bind.c",0x10e90,uVar2 & 0xffff,(dst->field_0).extaddr.addr_ref);
    return;
  }
  iVar6 = nwk_address_extended_by_ref((dst->field_0).extaddr.addr_ref,&uStack_18);
  if (iVar6 == 0) {
    log_write(3,"aps_bind.c",0x10ea4,uVar2 & 0xffff,uStack_18,dst_addr.field_0.u64._0_4_,
              (dst->field_0).extaddr.endpoint);
    return;
  }
  dst_addr_00 = "(nwk_address_extended_by_ref(dst->extaddr.addr_ref, &dst_addr)) == 0";
  uVar5 = 0x1c40;
  src_addr = (ezb_extaddr_t *)__assert_func("//build/esp-zigbee/src/core/aps/aps_bind.c",0x1c7);
