...
void nwk_mm_start_request(uint8_t iface_id,mac_start_req_t *req)

{
  ...
  if (iVar2 != 0) {
    ...
    pcVar3 = *(code **)(*(int *)(*(int *)(iVar2 + iVar1 * 0xc + 0xce8) + 4) + 0x24);
    iVar2 = core_globals_get();
    ...
    ;
    __assert_func("//build/esp-zigbee/src/core/nwk/nwk_multimac_proxy.c",0xba,"nwk_mm_start_request"
                  ,&_LC6);
  }
  __assert_func("//build/esp-zigbee/src/core/nwk/nwk_multimac_proxy.c",0xba,"nwk_mm_start_request",
                &_LC6);
  nwk_ed_mngr_handle_poll_indication();
  return;
}
