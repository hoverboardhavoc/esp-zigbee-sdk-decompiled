 ...
 * Source: libesp-zigbee-core.zczr.debug -> touchlink_commissioning.o -> ezb_zcl_touchlink_get_ep_list_cmd_req
 *
...
ezb_err_t ezb_zcl_touchlink_get_ep_list_cmd_req(ezb_zcl_touchlink_get_ep_list_cmd_req_t *cmd_req)

{
  ...
  packet._34_2_ = 0;
  if (cmd_req == (ezb_zcl_touchlink_get_ep_list_cmd_req_t *)0x0) {
    eVar1 = 2;
  }
  else {
    iVar2 = zcl_packet_init(&uStack_38,0);
    if (iVar2 == 0) {
      eVar1 = zcl_cmd_to_packet(&uStack_38,0,0,(cmd_req->cmd_ctrl).dis_default_rsp,0,0x42,0x1000);
      if (eVar1 == 0) {
