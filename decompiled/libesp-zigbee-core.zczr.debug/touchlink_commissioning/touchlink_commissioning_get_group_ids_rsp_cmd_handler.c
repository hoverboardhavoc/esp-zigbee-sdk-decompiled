...
void touchlink_commissioning_get_group_ids_rsp_cmd_handler(int param_1,int param_2)

{
  ...
  if (param_1 == 0) {
    ...
    if (uVar3 < auStack_22[0]) {
      ...
      pvStack_28 = calloc(uStack_2c >> 0x10 & 0xff,2);
      if (pvStack_28 == (void *)0x0) {
        uVar1 = 0x89;
      }
      else {
        for (uVar2 = 0; uVar2 < (uStack_2c >> 0x10 & 0xff); uVar2 = uVar2 + 1 & 0xff) {
          af_read_le16(*(undefined4 *)(param_1 + 0x24),auStack_22,
                       (void *)((int)pvStack_28 + uVar2 * 2));
        }
        if (uVar3 < auStack_22[0]) {
          uVar1 = 0x80;
        }
        else {
          uVar1 = zcl_message_group_ids_rsp(param_1,&uStack_2c);
        }
      }
    }
  }
