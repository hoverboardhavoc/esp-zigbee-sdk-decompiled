 ...
 * Source: libesp-zigbee-core.zczr.debug -> ias_zone.o -> ias_zone_zone_status_change_notif_cmd_req
 *
...
ezb_err_t ias_zone_zone_status_change_notif_cmd_req(ias_zone_status_change_notif_cmd_t *cmd_req)

{
  ...
  packet._34_2_ = 0;
  if (cmd_req == (ias_zone_status_change_notif_cmd_t *)0x0) {
    iVar1 = -1;
  }
  else {
    iVar1 = zcl_packet_init(&uStack_38,0);
    if ((iVar1 == 0) &&
       (iVar1 = zcl_cmd_to_packet(&uStack_38,0,1,(cmd_req->cmd_ctrl).dis_default_rsp,0,0,0x500),
       iVar1 == 0)) {
      uStack_3a = (cmd_req->payload).zone_status;
      iVar1 = zmsg_append_bytes(packet._32_4_,2,&uStack_3a);
      if (iVar1 == 0) {
        uStack_3a = CONCAT11(uStack_3a._1_1_,(cmd_req->payload).extend_status);
        iVar1 = zmsg_append_bytes(packet._32_4_,1,&uStack_3a);
        if (iVar1 == 0) {
          uStack_3a = CONCAT11(uStack_3a._1_1_,(cmd_req->payload).zone_id);
          iVar1 = zmsg_append_bytes(packet._32_4_,1,&uStack_3a);
          if (iVar1 == 0) {
            uStack_3a = (cmd_req->payload).delay;
            iVar1 = zmsg_append_bytes(packet._32_4_,2,&uStack_3a);
            if (iVar1 == 0) {
