 ...
 * Source: libesp-zigbee-core.zczr.debug -> level.o -> ezb_zcl_level_move_to_closest_frequency_cmd_req
 *
...
ezb_err_t ezb_zcl_level_move_to_closest_frequency_cmd_req
                    (ezb_zcl_level_move_to_closest_frequency_cmd_t *req)

{
  ...
  packet._34_2_ = 0;
  if (req == (ezb_zcl_level_move_to_closest_frequency_cmd_t *)0x0) {
    eVar1 = 2;
  }
  else {
    iVar2 = zcl_packet_init(&uStack_38,0);
    if (iVar2 == 0) {
      eVar1 = zcl_cmd_to_packet(&uStack_38,0,0,(req->cmd_ctrl).dis_default_rsp,0,8);
      if (eVar1 == 0) {
        uStack_3a = (req->payload).frequency;
