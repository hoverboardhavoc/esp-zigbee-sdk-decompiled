...

{
  ...
  template.write_attr_handler = electrical_measurement_cluster_srv_cmd_disc_handler;
  template.cmd_disc_handler = electrical_measurement_cluster_srv_cmd_proc_handler;
  iVar2 = zcl_cluster_template_add(&uStack_24);
  if (iVar2 == 0) {
    return;
  }
  pcVar1 = ezb_zcl_electrical_measurement_cluster_client_init;
  __assert_func("//build/esp-zigbee/src/core/api/zcl/cluster/electrical_measurement.c",0x12a,
                "ezb_zcl_electrical_measurement_cluster_server_init",
  ...
  __assert_func("//build/esp-zigbee/src/core/api/zcl/cluster/electrical_measurement.c",0x138,
                "ezb_zcl_electrical_measurement_cluster_client_init",
                "zcl_cluster_template_add(&template) == EZB_ZCL_STATUS_SUCCESS");
  return;
}
