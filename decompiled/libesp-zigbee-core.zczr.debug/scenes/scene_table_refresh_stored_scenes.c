...
void scene_table_refresh_stored_scenes(uint8_t ep_id)

{
  ...
  scene_table_remove_stored_all(ep_id);
  ...
  for (uVar1 = 0; uVar1 < pzVar3->total; uVar1 = uVar1 + 1 & 0xff) {
    ...
    if (CONCAT31(extraout_var,_Var2) != 0) {
      scene_table_store_scene(ep_id,(zcl_scene_table_entry_t *)(pzVar3 + uVar1 * 4 + 1));
    }
  }
  return;
}
