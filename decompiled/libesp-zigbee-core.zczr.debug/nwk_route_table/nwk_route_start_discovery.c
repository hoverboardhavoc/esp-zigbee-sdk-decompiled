...

{
  ...
  if (route->initiator == 0xff) {
    dst_addr = __assert_func("//builds/thread_zigbee/esp-zigbee/src/core/nwk/nwk_route_table.c",
                             0x104,"nwk_route_start_discovery","route->initiator < (0xff)");
    obj = nwk_route_table_find(dst_addr);
    if (obj != (nwk_route_t *)0x0) {
      if (obj->ref == '\0') {
        iVar3 = core_globals_get();
        blk_base = *(void **)(iVar3 + 0xc58);
        iVar3 = core_globals_get();
        blk_busy = *(bitmap_t **)(iVar3 + 0xc5c);
        iVar3 = core_globals_get();
        mempool_free_ent(blk_base,blk_busy,0x10,*(uint16_t *)(iVar3 + 0xc60),obj);
      }
      else {
