 ...
 * Source: libesp-zigbee-core.zczr.debug -> zcl_reporting.o -> zcl_reporting_remove_stored_reporting_info
 *
...

{
  ...
  while( true ) {
    ...
    if ((((auStack_30[1] == info->ep_id) && (itor.data._0_2_ == info->cluster_id)) &&
       ...
       ((CONCAT11((undefined1)itor.data.cluster_id,itor.data.profile_id._1_1_) == info->attr_id &&
        (((uint)itor.data._4_4_ >> 8 & 0xffff) == (uint)info->manuf_code)))) break;
    ds_zcl_reporting_info_next((ds_zcl_reporting_info_iterator_t *)auStack_30);
  }
  ds_zcl_reporting_info_delete((ds_zcl_reporting_info_iterator_t *)auStack_30);
  return;
}
