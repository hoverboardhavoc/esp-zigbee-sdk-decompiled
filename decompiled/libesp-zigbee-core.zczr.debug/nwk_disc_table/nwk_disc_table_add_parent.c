/*
 * Last changed at upstream commit 0dbfa9988ffc315d4d533fc462a8328b10d4d371
 * https://github.com/espressif/esp-zigbee-sdk/commit/0dbfa9988ffc315d4d533fc462a8328b10d4d371
 * Upstream date: 2026-07-09 09:00:50 +0000
 * Upstream subject: change: update esp-zigbee-lib (170bcb5a)
 * Source: libesp-zigbee-core.zczr.debug -> nwk_disc_table.o -> nwk_disc_table_add_parent
 *
 * (C) Espressif, Apache License 2.0.
 * Derivative work (this file): mechanical decompile via Ghidra (NSA, Apache 2.0).
 * Decompiler output may be incomplete or differ from original semantics.
 */

undefined4 nwk_disc_table_add_parent(void *param_1)

{
  int iVar1;
  int iVar2;
  undefined4 uVar3;
  void *__dest;
  
  iVar1 = core_globals_get();
  if (param_1 == (void *)0x0) {
    uVar3 = 2;
  }
  else {
    iVar2 = disc_table_find(iVar1 + 0xb2c,*(undefined1 *)((int)param_1 + 8),
                            *(uint *)((int)param_1 + 0x10) >> 0x1d & 1,param_1);
    if (iVar2 == 0) {
      __dest = (void *)disc_table_new(iVar1 + 0xb2c);
      if ((__dest == (void *)0x0) &&
         (__dest = (void *)disc_table_find_weakest(*(undefined1 *)((int)param_1 + 9)),
         __dest == (void *)0x0)) {
        uVar3 = 1;
      }
      else {
        memcpy(__dest,param_1,0x14);
        uVar3 = 0;
      }
    }
    else {
      uVar3 = 0;
    }
  }
  return uVar3;
}

