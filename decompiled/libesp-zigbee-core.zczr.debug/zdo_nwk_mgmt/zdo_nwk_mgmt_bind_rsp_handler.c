...
zdp_status_t zdo_nwk_mgmt_bind_rsp_handler(zdo_packet_t *packet)

{
  ...
  if (packet == (zdo_packet_t *)0x0) {
    ...
    zVar1 = zdo_op_nwk_mgmt_bind_rsp
                      (packet->payload,(zdp_nwk_mgmt_bind_rsp_field_t *)auStack_18,false);
    if (CONCAT31(extraout_var,zVar1) == 0) {
      zVar1 = zdo_cb_nwk_mgmt_bind_rsp((zdp_nwk_mgmt_bind_rsp_field_t *)auStack_18,&packet->ctx);
    }
    else {
      zVar1 = 0xfe;
    }
