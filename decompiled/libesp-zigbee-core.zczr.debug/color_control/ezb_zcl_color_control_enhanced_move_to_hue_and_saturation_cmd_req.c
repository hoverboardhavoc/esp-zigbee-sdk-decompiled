 ...
 * Source: libesp-zigbee-core.zczr.debug -> color_control.o -> ezb_zcl_color_control_enhanced_move_to_hue_and_saturation_cmd_req
 *
...
ezb_err_t ezb_zcl_color_control_enhanced_move_to_hue_and_saturation_cmd_req
                    (ezb_zcl_color_control_enhanced_move_to_hue_and_saturation_cmd_t *cmd_req)

{
  ...
  packet._34_2_ = 0;
  if (cmd_req == (ezb_zcl_color_control_enhanced_move_to_hue_and_saturation_cmd_t *)0x0) {
    eVar1 = -1;
  }
  else {
    iVar2 = zcl_packet_init(&uStack_38,0);
    if (iVar2 == 0) {
      eVar1 = zcl_cmd_to_packet(&uStack_38,0,0,(cmd_req->cmd_ctrl).dis_default_rsp,0,0x43,0x300);
      if (eVar1 == 0) {
        uStack_3a = (cmd_req->payload).enhanced_hue;
        eVar1 = zmsg_append_bytes(packet._32_4_,2,&uStack_3a);
        if (eVar1 == 0) {
          uStack_3a = CONCAT11(uStack_3a._1_1_,(cmd_req->payload).saturation);
