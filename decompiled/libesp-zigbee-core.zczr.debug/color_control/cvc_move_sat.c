...
ezb_zcl_status_t cvc_move_sat(uint8_t ep_id,ezb_zcl_color_control_move_hue_cmd_payload_t *payload)

{
  ...
  if (payload == (ezb_zcl_color_control_move_hue_cmd_payload_t *)0x0) {
    __assert_func("//build/esp-zigbee/src/core/api/zcl/cluster/color_control.c",0x371,"cvc_move_sat"
                  ,"payload");
  }
  else {
    input.ep_id = '\0';
    ...
    if (uVar1 != '\x01') {
      ...
      input.begin = 0;
      input._12_4_ = (((uint)(ushort)input.end * 0xa0000 >> 0x10) * in_a5) / (uint)payload->rate;
      goto _L0;
    }
  }
  if (payload->rate == '\0') {
  ...
  input.begin = 0xfe;
  input._12_4_ = (int)(((uint)(ushort)input.end * 0xa0000 >> 0x10) * (0xfe - in_a5)) /
