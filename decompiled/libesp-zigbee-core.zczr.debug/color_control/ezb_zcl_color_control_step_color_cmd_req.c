 ...
 * Source: libesp-zigbee-core.zczr.debug -> color_control.o -> ezb_zcl_color_control_step_color_cmd_req
 *
...

{
  ...
  if (cmd_req == (ezb_zcl_color_control_step_color_cmd_t *)0x0) {
    ...
    if (iVar2 == 0) {
      eVar1 = zcl_cmd_to_packet(&uStack_38,0,0,(cmd_req->cmd_ctrl).dis_default_rsp,0,9,0x300);
      if (eVar1 == 0) {
        uStack_3a = (uint16_t)(cmd_req->payload).step_x;
        eVar1 = zmsg_append_bytes(packet._32_4_,2,&uStack_3a);
        if (eVar1 == 0) {
          uStack_3a = (uint16_t)(cmd_req->payload).step_y;
          eVar1 = zmsg_append_bytes(packet._32_4_,2,&uStack_3a);
          if (eVar1 == 0) {
