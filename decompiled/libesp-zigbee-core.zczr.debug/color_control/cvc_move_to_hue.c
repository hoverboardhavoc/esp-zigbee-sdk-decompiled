...
ezb_zcl_status_t
cvc_move_to_hue(uint8_t ep_id,ezb_zcl_color_control_move_to_hue_cmd_payload_t *payload)

{
  ...
  if (payload == (ezb_zcl_color_control_move_to_hue_cmd_payload_t *)0x0) {
    __assert_func("//build/esp-zigbee/src/core/api/zcl/cluster/color_control.c",0x2e8,
...
_L0:
    ...
    input._0_4_ = ZEXT14(*pzVar3->data_p);
    input.begin = (int32_t)payload->hue;
    bVar1 = payload->direction;
    if (bVar1 == 2) {
      if ((uint)input.begin < (uint)input._0_4_) {
        input.begin = input.begin + 0xff;
      }
    }
    else if (bVar1 < 3) {
      if (bVar1 == 0) {
        ...
        if ((uint)input._0_4_ < (uint)input.begin == 0x7f < iVar4) {
