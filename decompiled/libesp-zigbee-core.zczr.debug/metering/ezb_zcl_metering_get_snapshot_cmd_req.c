 ...
 * Source: libesp-zigbee-core.zczr.debug -> metering.o -> ezb_zcl_metering_get_snapshot_cmd_req
 *
...

{
  ...
  if (cmd_req == (ezb_zcl_metering_get_snapshot_cmd_t *)0x0) {
    ...
    if (iVar2 == 0) {
      eVar1 = zcl_cmd_to_packet(auStack_3c + 1,0,0,(cmd_req->cmd_ctrl).dis_default_rsp,0,6,0x702);
      if (eVar1 == 0) {
        auStack_3c[0] = (cmd_req->payload).earliest_start_time;
        eVar1 = zmsg_append_bytes(packet._32_4_,4,auStack_3c);
        if (eVar1 == 0) {
          auStack_3c[0] = (cmd_req->payload).latest_end_time;
          eVar1 = zmsg_append_bytes(packet._32_4_,4,auStack_3c);
          if (eVar1 == 0) {
            auStack_3c[0] = CONCAT31(auStack_3c[0]._1_3_,(cmd_req->payload).snapshot_offset);
            eVar1 = zmsg_append_bytes(packet._32_4_,1,auStack_3c);
            if (eVar1 == 0) {
              auStack_3c[0] = (cmd_req->payload).snapshot_cause;
              eVar1 = zmsg_append_bytes(packet._32_4_,4,auStack_3c);
              if (eVar1 == 0) {
