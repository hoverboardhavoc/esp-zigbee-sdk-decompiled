 ...
 * Source: libesp-zigbee-core.zczr.debug -> metering.o -> ezb_zcl_metering_get_sampled_data_cmd_req
 *
...
ezb_err_t ezb_zcl_metering_get_sampled_data_cmd_req
                    (ezb_zcl_metering_get_sampled_data_cmd_t *cmd_req)

{
  ...
  packet._34_2_ = 0;
  if (cmd_req == (ezb_zcl_metering_get_sampled_data_cmd_t *)0x0) {
    eVar1 = -1;
  }
  else {
    iVar2 = zcl_packet_init(auStack_3c + 1,0);
    if (iVar2 == 0) {
      eVar1 = zcl_cmd_to_packet(auStack_3c + 1,0,0,(cmd_req->cmd_ctrl).dis_default_rsp,0,8,0x702);
      if (eVar1 == 0) {
        auStack_3c[0] = CONCAT22(auStack_3c[0]._2_2_,(cmd_req->payload).sample_id);
        eVar1 = zmsg_append_bytes(packet._32_4_,2,auStack_3c);
        if (eVar1 == 0) {
          auStack_3c[0] = (cmd_req->payload).earliest_start_time;
          eVar1 = zmsg_append_bytes(packet._32_4_,4,auStack_3c);
          if (eVar1 == 0) {
            auStack_3c[0] = CONCAT31(auStack_3c[0]._1_3_,(cmd_req->payload).sample_type);
            eVar1 = zmsg_append_bytes(packet._32_4_,1,auStack_3c);
            if (eVar1 == 0) {
              auStack_3c[0] = CONCAT22(auStack_3c[0]._2_2_,(cmd_req->payload).num_of_samples);
              eVar1 = zmsg_append_bytes(packet._32_4_,2,auStack_3c);
              if (eVar1 == 0) {
