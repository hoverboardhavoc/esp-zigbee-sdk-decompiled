 ...
 * Source: libesp-zigbee-core.zczr.debug -> metering.o -> ezb_zcl_metering_request_fast_poll_mode_cmd_req
 *
...

{
  ...
  if (cmd_req == (ezb_zcl_metering_request_fast_poll_mode_cmd_t *)0x0) {
    ...
    if (iVar2 == 0) {
      eVar1 = zcl_cmd_to_packet(&uStack_38,0,0,(cmd_req->cmd_ctrl).dis_default_rsp,0,3,0x702);
      if (eVar1 == 0) {
        uStack_39 = (cmd_req->payload).period;
        eVar1 = zmsg_append_bytes(packet._32_4_,1,&uStack_39);
        if (eVar1 == 0) {
          uStack_39 = (cmd_req->payload).duration;
          eVar1 = zmsg_append_bytes(packet._32_4_,1,&uStack_39);
          if (eVar1 == 0) {
