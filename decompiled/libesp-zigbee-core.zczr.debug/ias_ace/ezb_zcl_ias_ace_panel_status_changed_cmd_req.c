 ...
 * Source: libesp-zigbee-core.zczr.debug -> ias_ace.o -> ezb_zcl_ias_ace_panel_status_changed_cmd_req
 *
...

{
  ...
  if (cmd_req == (ezb_zcl_ias_ace_panel_status_changed_cmd_t *)0x0) {
    ...
    if ((iVar1 == 0) &&
       (iVar1 = zcl_cmd_to_packet(&uStack_38,0,1,(cmd_req->cmd_ctrl).dis_default_rsp,0,4,0x501),
       iVar1 == 0)) {
      uStack_39 = (cmd_req->payload).panel_status;
      iVar1 = zmsg_append_bytes(packet._32_4_,1,&uStack_39);
      if (iVar1 == 0) {
        uStack_39 = (cmd_req->payload).seconds_remaining;
        iVar1 = zmsg_append_bytes(packet._32_4_,1,&uStack_39);
        if (iVar1 == 0) {
          uStack_39 = (cmd_req->payload).aud_notif;
          iVar1 = zmsg_append_bytes(packet._32_4_,1,&uStack_39);
          if (iVar1 == 0) {
            uStack_39 = (cmd_req->payload).alarm_status;
            iVar1 = zmsg_append_bytes(packet._32_4_,1,&uStack_39);
            if (iVar1 == 0) {
