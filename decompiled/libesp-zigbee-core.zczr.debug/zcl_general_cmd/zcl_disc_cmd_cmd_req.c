  ...
  if (cmd_req == (zcl_disc_cmd_cmd_t *)0x0) {
    ...
    if (iVar1 == 0) {
      ...
      iVar1 = zcl_cmd_to_packet(&uStack_38,0,uVar2 & 1,uVar2 >> 1 & 1,uVar2 >> 2 & 1,
                                (cmd_req->cmd_ctrl).manuf_code,uVar3,(cmd_req->cmd_ctrl).cluster_id)
      ;
      if (iVar1 == 0) {
        uStack_39 = (uint8_t)(cmd_req->payload).start_cmd_id;
        iVar1 = zmsg_append_bytes(packet._32_4_,1,&uStack_39);
        if (iVar1 == 0) {
          uStack_39 = (cmd_req->payload).max_cmd_number;
          iVar1 = zmsg_append_bytes(packet._32_4_,1,&uStack_39);
          if (iVar1 == 0) {
