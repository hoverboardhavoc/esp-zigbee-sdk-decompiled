 ...
 * Source: libesp-zigbee-core.zczr.debug -> zcl_general_cmd.o -> zcl_default_rsp_cmd_req
 *
...
ezb_err_t zcl_default_rsp_cmd_req(zcl_default_rsp_cmd_t *cmd_req)

{
  ...
  packet._34_2_ = 0;
  if (cmd_req == (zcl_default_rsp_cmd_t *)0x0) {
    eVar1 = -1;
  }
  else {
    iVar2 = zcl_packet_init(&uStack_38,*(uint *)&(cmd_req->cmd_ctrl).fc & 1);
    if (iVar2 == 0) {
      uVar3 = *(uint *)&(cmd_req->cmd_ctrl).fc;
      eVar1 = zcl_cmd_to_packet(&uStack_38,0,uVar3 & 1,uVar3 >> 1 & 1,uVar3 >> 2 & 1,
                                (cmd_req->cmd_ctrl).manuf_code,0xb,(cmd_req->cmd_ctrl).cluster_id);
      if (eVar1 == 0) {
        uStack_39 = (cmd_req->payload).rsp_to_cmd;
        eVar1 = zmsg_append_bytes(packet._32_4_,1,&uStack_39);
        if (eVar1 == 0) {
          uStack_39 = (cmd_req->payload).status_code;
          eVar1 = zmsg_append_bytes(packet._32_4_,1,&uStack_39);
          if (eVar1 == 0) {
            packet.header._20_4_ = packet.header._20_4_ | 0x200000;
            packet.header.fc = (cmd_req->payload).tsn;
