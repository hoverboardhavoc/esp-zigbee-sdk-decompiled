 ...
 * Source: libesp-zigbee-core.zczr.debug -> groups.o -> ezb_zcl_groups_add_group_cmd_req
 *
...

{
  ...
  if (cmd_req == (ezb_zcl_groups_add_group_cmd_t *)0x0) {
    ...
    if (iVar1 == 0) {
      iVar1 = zcl_cmd_to_packet(&uStack_38,0,0,(cmd_req->cmd_ctrl).dis_default_rsp,0,0,4);
      if (iVar1 == 0) {
        ...
        if (iVar1 == 0) {
          if ((cmd_req->payload).group_name == (char *)0x0) {
            ...
            iVar1 = zcl_packet_append_variable_attr_value(packet._32_4_,0x42);
          }
          if (iVar1 == 0) {
