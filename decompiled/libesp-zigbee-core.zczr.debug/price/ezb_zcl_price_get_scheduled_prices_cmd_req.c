 ...
 * Source: libesp-zigbee-core.zczr.debug -> price.o -> ezb_zcl_price_get_scheduled_prices_cmd_req
 *
...

{
  ...
  if (cmd_req == (ezb_zcl_price_get_scheduled_prices_cmd_t *)0x0) {
    ...
    if (iVar2 == 0) {
      eVar1 = zcl_cmd_to_packet(&uStack_38,0,0,(cmd_req->cmd_ctrl).dis_default_rsp,0,1,0x700);
      if (eVar1 == 0) {
        uStack_3c = (cmd_req->payload).start_time;
        eVar1 = zmsg_append_bytes(packet._32_4_,4,&uStack_3c);
        if (eVar1 == 0) {
          uStack_3c = CONCAT31(uStack_3c._1_3_,(cmd_req->payload).num_of_events);
