 ...
 * Source: libesp-zigbee-core.zczr.debug -> on_off.o -> on_off_send_no_payload_cmd
 *
...
ezb_err_t on_off_send_no_payload_cmd(ezb_zcl_on_off_cmd_t *cmd_req,uint8_t cmd_id)

{
  ...
  if (cmd_req == (ezb_zcl_on_off_cmd_t *)0x0) {
    ...
    if (iVar2 == 0) {
      eVar1 = zcl_cmd_to_packet(&uStack_38,0,0,(cmd_req->cmd_ctrl).dis_default_rsp,0,
                                CONCAT31(in_register_0000202d,cmd_id),6);
      if (eVar1 == 0) {
