 ...
 * Source: libesp-zigbee-core.zczr.debug -> touchlink_cmd.o -> zcl_touchlink_identify_req
 *
...
ezb_err_t zcl_touchlink_identify_req(zcl_touchlink_identify_req_t *cmd_req)

{
  ...
  packet._34_2_ = 0;
  if (cmd_req == (zcl_touchlink_identify_req_t *)0x0) {
    eVar1 = -1;
  }
  else {
    iVar2 = zcl_packet_init(auStack_38,0);
    if (iVar2 == 0) {
      eVar1 = zcl_touchlink_cmd_to_packet((zcl_packet_t *)auStack_38,'\0','\x06',unaff_s1);
      if (eVar1 == 0) {
        uStack_3c = cmd_req->transaction_id;
        eVar1 = zmsg_append_bytes(packet._32_4_,4,&uStack_3c);
        if (eVar1 == 0) {
          uStack_3c = CONCAT22(uStack_3c._2_2_,cmd_req->duration);
