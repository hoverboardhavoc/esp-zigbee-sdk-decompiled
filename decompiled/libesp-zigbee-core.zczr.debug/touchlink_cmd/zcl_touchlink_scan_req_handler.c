 ...
 * Source: libesp-zigbee-core.zczr.debug -> touchlink_cmd.o -> zcl_touchlink_scan_req_handler
 *
...
zcl_status_t zcl_touchlink_scan_req_handler(zcl_packet_t *packet)

{
  ...
  if (packet == (zcl_packet_t *)0x0) {
    ...
    uStack_2e = 0;
    af_read_le32(packet->payload,&uStack_2e,(uint32_t *)((int)&req.cmd_ctrl.peer_addr.u + 6));
    af_read_le8(packet->payload,&uStack_2e,(uint8_t *)&req.transaction_id);
    af_read_le8(packet->payload,&uStack_2e,(uint8_t *)((int)&req.transaction_id + 1));
    uVar3 = zmsg_get_length(packet->payload);
    if (uVar3 < uStack_2e) {
      zVar2 = 0x80;
    }
    else {
      uVar1 = touchlink_scan_req_handler(&stack0xffffffd4);
