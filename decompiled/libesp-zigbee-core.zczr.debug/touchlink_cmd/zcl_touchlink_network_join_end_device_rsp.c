/*
 * Last changed at upstream commit 02e71c61b42f2e0f80074362fea601f2245ed9d0
 * https://github.com/espressif/esp-zigbee-sdk/commit/02e71c61b42f2e0f80074362fea601f2245ed9d0
 * Upstream date: 2026-04-16 12:25:02 +0800
 * Upstream subject: change: update esp-zigbee-lib 2.x (bce53822)
 * Source: libesp-zigbee-core.zczr.debug -> touchlink_cmd.o -> zcl_touchlink_network_join_end_device_rsp
 *
 * (C) Espressif, Apache License 2.0.
 * Derivative work (this file): mechanical decompile via Ghidra (NSA, Apache 2.0).
 * Decompiler output may be incomplete or differ from original semantics.
 */

/* WARNING: Unknown calling convention */

ezb_err_t zcl_touchlink_network_join_end_device_rsp(zcl_touchlink_join_end_device_rsp_t *cmd_rsp)

{
  ezb_address_t *unaff_s1;
  ezb_err_t eVar1;
  int iVar2;
  uint32_t uStack_3c;
  undefined1 auStack_38 [4];
  zcl_packet_t packet;
  
  auStack_38 = (undefined1  [4])0x0;
  packet.header.src_addr._0_4_ = 0;
  packet.header.src_addr.u._2_4_ = 0;
  packet.header._8_4_ = 0;
  packet.header.dst_addr.u._0_2_ = 0;
  packet.header.dst_addr.u._2_2_ = 0;
  packet.header.dst_addr.u._4_4_ = 0;
  packet.header.src_ep = '\0';
  packet.header.dst_ep = '\0';
  packet.header.cluster_id = 0;
  packet.header.profile_id = 0;
  packet.header.fc = '\0';
  packet.header._27_1_ = 0;
  packet.header.manuf_code = 0;
  packet.header.tsn = '\0';
  packet.header.rssi = '\0';
  packet.header.cmd_id = '\0';
  packet.header._33_1_ = 0;
  packet._34_2_ = 0;
  if (cmd_rsp == (zcl_touchlink_join_end_device_rsp_t *)0x0) {
    eVar1 = -1;
  }
  else {
    iVar2 = zcl_packet_init(auStack_38,0);
    if (iVar2 == 0) {
      eVar1 = zcl_touchlink_cmd_to_packet((zcl_packet_t *)auStack_38,'\x01','\x15',unaff_s1);
      if (eVar1 == 0) {
        packet.header.fc = (cmd_rsp->cmd_ctrl).tsn;
        packet.header._20_4_ = packet.header._20_4_ | 0x200000;
        uStack_3c = cmd_rsp->transaction_id;
        eVar1 = zmsg_append_bytes(packet._32_4_,4,&uStack_3c);
        if (eVar1 == 0) {
          uStack_3c = CONCAT31(uStack_3c._1_3_,cmd_rsp->status);
          eVar1 = zmsg_append_bytes(packet._32_4_,1,&uStack_3c);
          if (eVar1 == 0) {
            zcl_packet_intrp_send(auStack_38,&cmd_rsp->cnf_ctx);
            eVar1 = zcl_status_to_err();
            if (eVar1 == 0) {
              return 0;
            }
          }
        }
      }
    }
    else {
      eVar1 = -1;
    }
  }
  zcl_packet_free(auStack_38);
  return eVar1;
}

