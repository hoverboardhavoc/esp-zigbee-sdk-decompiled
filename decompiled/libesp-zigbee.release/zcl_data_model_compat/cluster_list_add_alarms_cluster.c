 ...
 * Source: libesp-zigbee.release -> zcl_data_model_compat.o -> cluster_list_add_alarms_cluster
 *
...
esp_err_t cluster_list_add_alarms_cluster
...

{
  ...
  if ((attr_list != (esp_zb_attribute_list_t *)0x0) &&
    ...
    *(uint8_t *)((int)&attr_list[-3].attr_list + 2) = role_mask;
    if ((role_mask & 1) == 0) {
      puVar3 = &ezb_zcl_alarms_cluster_client_init;
    }
    else {
      puVar3 = &ezb_zcl_alarms_cluster_server_init;
    }
    attr_list[-1].attr_list = puVar3;
    zcl_cluster_desc_add_revision_attr(cluster_desc,1);
    if ((role_mask & 1) != 0) {
      uStack_12 = 0xf;
      ezb_zcl_cluster_desc_add_manuf_attr(cluster_desc,0xeffe,0x21,0x40,0x131b,&uStack_12);
      ezb_zcl_cluster_desc_add_manuf_attr(cluster_desc,0xefff,0,0x40,0x131b,0);
    }
