 ...
 * Source: libesp-zigbee.release -> zcl_data_model_compat.o -> cluster_list_add_temperature_measurement_cluster
 *
...
esp_err_t cluster_list_add_temperature_measurement_cluster
...

{
  ...
  if ((attr_list != (esp_zb_attribute_list_t *)0x0) &&
    ...
    *(uint8_t *)((int)&attr_list[-3].attr_list + 2) = role_mask;
    if ((role_mask & 1) == 0) {
      puVar3 = &ezb_zcl_temperature_measurement_cluster_client_init;
    }
    else {
      puVar3 = &ezb_zcl_temperature_measurement_cluster_server_init;
    }
    attr_list[-1].attr_list = puVar3;
    zcl_cluster_desc_add_revision_attr((zcl_cluster_desc_t *)(attr_list + -3),3);
